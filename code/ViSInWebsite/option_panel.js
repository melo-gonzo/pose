...
export async function setupDatGui() {
  ...
  const facingModeController = cameraFolder.add(
    params.STATE.camera,
    "cameraDirection",
    Object.keys(params.FACING_MODES)
  );
  facingModeController.onChange((_) => {
    params.STATE.isFacingModeOptionChanged = true;
  });
  ...
  const model = "movenet"; //urlParams.get("model");
  let type = "thunder"; //urlParams.get("type");
  ...
  console.log(params);
  console.log("option_panel.js line 83 blazepose");
...
// The facing mode config folder contains which way to point the camera
// function addFacingModeControllers(modelConfigFolder) {
//   params.STATE.modelConfig = {params.FACING_MODE_CONFIG};
